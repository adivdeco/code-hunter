...
const ProductCard = ({ item, onAddToCart }) => (
    <motion.div
        ...
        exit={{ opacity: 0, scale: 0.8 }}
        transition={{ type: "spring", stiffness: 200, damping: 25 }}
        className="relative group overflow-hidden rounded-xl bg-black/40 border border-white/10 backdrop-blur-lg shadow-lg flex flex-col p-4"
    >
        {item.tag && (<div className="absolute top-4 right-4 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-xs font-bold px-3 py-1 rounded-full z-10 shadow-md flex items-center gap-1"><Zap className="w-3 h-3" />{item.tag}</div>)}
        ...
        <div className="pt-4 flex flex-col flex-grow">
            <p className="text-sm text-purple-400 font-medium">{item.category}</p>
            <h3 className="text-xl font-bold text-white mt-1 truncate flex-grow">{item.name}</h3>
            <div className="flex justify-between items-center mt-4">
                <p className="text-2xl font-semibold text-white">₹{item.price.toLocaleString('en-IN')}</p>
                <button
                    onClick={() => onAddToCart(item)}
                    className="flex items-center gap-2 bg-white/10 text-white px-4 py-2 rounded-lg hover:bg-white/20 transition-colors border border-white/20"
                >
                    <ShoppingBag className="w-5 h-5" /><span>Cart</span>
                </button>
            </div>
...
const ShopPage = () => {
    ...
    // STEP 2: LOAD CART FROM LOCALSTORAGE ON INITIAL RENDER
    const [cart, setCart] = useState(() => {
        try {
    ...
    // Effect for filtering items based on category
    useEffect(() => {
        if (activeCategory === 'All') {
            setFilteredItems(merchandiseData);
        } else {
    ...
    const handleAddToCart = (item) => {
        ...
        toast.success(`${item.name} added to cart!`, { icon: '🛒', style: { background: '#1a1a2e', color: '#e0e0e0', border: '1px solid rgba(255, 255, 255, 0.1)' } });
    };
    ...
    const handleRemoveFromCart = (id) => {
        setCart(prevCart => prevCart.filter(item => item.id !== id));
        toast.error("Item removed from cart.", { style: { background: '#1a1a2e', color: '#e0e0e0', border: '1px solid rgba(255, 255, 255, 0.1)' } });
    };

    // STEP 4: CREATE THE CHECKOUT HANDLER
    const handleCheckout = () => {
        if (cart.length === 0) {
            toast.error("Your cart is empty!");
            return;
        }
        setIsCartOpen(false); // Close the flyout
        navigate('/pay', { state: { cartItems: cart } });
    };
    ...
    return (
        <div className="relative min-h-screen w-full bg-gray-900 text-white font-sans overflow-x-hidden">
            ...
            {/* STEP 5: PASS THE NEW CHECKOUT HANDLER TO THE FLYOUT */}
            <CartFlyout
                isOpen={isCartOpen}
                ...
                onCheckout={handleCheckout} // <-- Pass the handler
            />
            ...
            >
                ...
                {cartItemCount > 0 &&
                    <motion.span
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="absolute -top-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-white text-purple-600 text-xs font-bold"
                    >
                        {cartItemCount}
                    </motion.span>
                }
            ...
            <main className="relative z-10 pt-24 md:pt-32 pb-20 mb-[40vh]">
                ...
                {/* --- Animated Filter Buttons --- */}
                <div className="flex justify-center my-12 px-4">
                    <div className="flex flex-wrap justify-center gap-2 p-1.5 rounded-full bg-black/30 border border-white/10">
                        {categories.map((cat) => (
                            <button key={cat} onClick={() => setActiveCategory(cat)} className={`relative flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeCategory === cat ? 'text-white' : 'text-white/60 hover:text-white'}`}>
                                {activeCategory === cat && (
                                    <motion.div layoutId="activePill" className="absolute inset-0 bg-white/10 rounded-full" style={{ borderRadius: 9999 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} />
