...
export const loginUser = createAsyncThunk('auth/login', async (credentials, { rejectWithValue }) => {
  try {
    await axiosClient.post('/auth/login', credentials, { withCredentials: true });
    const { data } = await axiosClient.get('/auth/check', { withCredentials: true });
    return data?.user;
  } catch (error) {
    if (error.response) {
      ...
      return rejectWithValue({ message: 'Error', details: error.message });
    }
  }
});
...
const authSlice = createSlice({
  ...
  reducers: {}, // ✅ Reducers not needed because thunks do everything
  extraReducers: (builder) => {
    builder
      // register
      .addCase(registerUser.pending, (state) => {
