...
const register = async (req, res) => {
    try {
        ...
        const token = createToken({ _id: user._id, email, role: 'user' });
...
const login = async (req, res) => {
    try {
        ...
        const token = createToken({ email, _id: user._id, role: user.role });
        ...
        res.status(200).json({ success: true, message: 'Login successful', user: reply });
